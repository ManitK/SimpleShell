...
int flag_for_allocating_memory=0;
  int page_size = 4096;
  // Starting page address of the segment where the seg fault occurred
  uintptr_t page_start = seg_fault_address & -page_size;

  // Allocate memory for one page
  void *allocated_memory = mmap((void *)page_start, page_size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  ...
  // offset within segment where seg fault occured
  off_t segment_offset = segment->p_offset + (seg_fault_address - page_start);
  // size left in segment after seg fault address
  ssize_t remaining_segment_size = segment->p_memsz - (seg_fault_address - page_start);
  //going to offset where segfault occured in segment
  lseek(fd, segment_offset, SEEK_SET);

  ssize_t bytes_to_read;
  // only read what is necessary in the page
  if(remaining_segment_size >= page_size){
   bytes_to_read = page_size;
   read(fd, allocated_memory,page_size);
  }
  else{
   bytes_to_read = remaining_segment_size;
   read(fd, allocated_memory,remaining_segment_size);
  }

  //the internal fragmentation size for every page allocated
  overall_segment_size = overall_segment_size + (page_size - bytes_to_read);
}
